...
namespace PTTest
{
    public partial class JacobiForm : Form
    {
        ...
        private TextBox textBoxTolerableError;
...
>>>>>>> 10b2366 (fix some bugs)
            ...
            // TextBox for tolerable error
            this.textBoxTolerableError = new TextBox();
            this.textBoxTolerableError.Size = new Size(textBoxSize, textBoxSize);
            this.textBoxTolerableError.Location = new Point(100, 197);
            this.Controls.Add(this.textBoxTolerableError);
            Label labelTolerableError = new Label();
            labelTolerableError.Text = "Tolerable Error";
            labelTolerableError.Location = new Point(20, 200);
            this.Controls.Add(labelTolerableError);
        ...
        private void GenerateMatrix()
        {
            ...
            // Clear existing text boxes in the initial x values
            ...
            for (int i = 0; i < count; i++)
            {
                TextBox textBox = new TextBox();
                textBox.Size = new Size(textBoxSize, textBoxSize);
                textBox.Location = new Point(170 + count * (textBoxSize + textBoxMargin) + 100, 70 + i * (textBoxSize + textBoxMargin)); // Add extra margin for initial x values
                textBoxesInitialX.Add(textBox);
                this.Controls.Add(textBox);
            }

            //  Label labelInitialX = new Label();
            //labelInitialX.Text = "Initial X:";
            //labelInitialX.Location = new Point(170 + count * (textBoxSize + textBoxMargin) + 100, 180);
            // this.Controls.Add(labelInitialX);
        }
...
>>>>>>> 10b2366 (fix some bugs)
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            ...
            float tolerableError = 0.0001f; // Default tolerable error
            int maxIterations = 1000; // Maximum iterations to prevent infinite loop
            if (click != 0)
            {
                ...
                using (Process process = new Process())
                {
                    ...
                    try
                    {
                        ...
                        tolerableError = float.Parse(textBoxTolerableError.Text);
                        ...
                        for (int m = 0; m < maxIterations; m++)
                        {
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                newX[i] = sum / a[i, i];
                                if (Math.Abs(newX[i] - x[i]) > tolerableError)
                                {
                                    hasConverged = false;
                                }
                            }

                            // Update x with newX values
                            for (int i = 0; i < count; i++)
                            {
                                x[i] = newX[i];
                            }

                            // Add current iteration results to DataGridView
                        ...
                        // Optionally append final solution
                        var finalRow = new object[count + 1];
                        finalRow[0] = "Solution";
                        for (int i = 0; i < count; i++)
                        {
                            finalRow[i + 1] = $"x{i + 1} = {x[i]:F6}";
                        }
                        resultsGridView.Rows.Add(finalRow);
                    }
        ...
        private void JacobiForm_Load(object sender, EventArgs e)
        {
